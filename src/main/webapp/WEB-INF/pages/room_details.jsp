...
<div class="container">
    ...
    <h3>Building Name : ${room.building.buildingName}</h3>

    <c:if test="${not empty itemsInTheRoom}">
        <table class="table table-striped">
            <tr>
                <th>Item ID</th>
                <th>Item Name</th>
                <th>Item Allotment</th>
            </tr>
            <c:forEach items="${itemsInTheRoom}" var="itemInTheRoom">
                <tr>
                    <td>
                        <a href="<c:url value="/items/details/${itemInTheRoom.pk.item.itemId}"/>">${itemInTheRoom.pk.item.itemId}</a>
                    </td>
                    <td>${itemInTheRoom.pk.item.itemName}</td>
                    <td>${itemInTheRoom.itemMaxAllotment}</td>
                </tr>
            </c:forEach>
        </table>
